...
export default function ReportsTelemetry() {
  ...
  const avgBasket =
    invoices.length > 0 ? Math.round(totalSales / invoices.length) : 1850;
    ...
    <div className="max-w-[1200px] mx-auto space-y-6 select-none font-sans">
      ...
      <div>
        <h1 className="font-display text-2xl font-bold text-[#191c1e] flex items-center gap-2">
          <BarChart3 className="text-[#0f3d57] h-6 w-6" /> Operations & Sales
          Reports
        </h1>
        <p className="text-sm text-[#42474d] mt-1">
          Detailed overview of transactions, average ticket sizes, and revenue
          metrics.
        </p>
      </div>
      ...
      <div className="bg-white border border-[#eceef1] rounded-xl p-6 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
        <h3 className="text-base font-bold text-[#191c1e] mb-4 font-display">
              ...
              <tbody className="text-sm divide-y divide-[#eceef1] text-[#191c1e]">
                {invoices.map((inv: any) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-semibold text-right text-[#42474d]">
                      {formatLKR(
                        inv.total_cost ?? inv.net_amount - inv.net_profit
                      )}
                    </td>
                    <td className="p-4 font-semibold text-right text-emerald-600">
