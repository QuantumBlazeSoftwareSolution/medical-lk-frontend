...
function stockStatus(qty: number, minLevel: number) {
  ...
  if (qty <= minLevel)
    return {
      ...
      bg: 'bg-[#fef9ec]',
      bar: 'bg-[#e67e22]',
      pct: Math.max(5, (qty / (minLevel * 3)) * 100),
    };
  return {
    label: 'In Stock',
    color: 'text-[#006d37]',
    bg: 'bg-[#e8f8f5]',
    bar: 'bg-[#006d37]',
    pct: Math.min(100, (qty / (minLevel * 5)) * 100),
  };
}
...
function batchExpiry(dateStr: string) {
  ...
  if (d < 0)
    return {
      color: 'text-[#ba1a1a] font-semibold',
      badge: 'bg-[#ffdad6] text-[#ba1a1a]',
      label: 'Expired',
    };
  if (d <= 30)
    return {
      color: 'text-[#b45309] font-semibold',
      badge: 'bg-[#fef3c7] text-[#b45309]',
      label: 'Expiring Soon',
    };
  return {
    color: 'text-[#42474d]',
    badge: 'bg-[#eceef1] text-[#42474d]',
    label: 'OK',
  };
}
...
export default function InventoryPage() {
  ...
  const handleSelectNMRAProduct = (prod: any) => {
    ...
    if (dosage.includes('tablet')) {
      ...
      dosage.includes('syrup') ||
      dosage.includes('liquid') ||
      dosage.includes('suspension')
    ) {
      setMedCategory('syrup');
      setMedUom('bottle');
    } else if (
      dosage.includes('injection') ||
      dosage.includes('vial') ||
      dosage.includes('ampoule')
    ) {
      setMedCategory('injection');
      setMedUom('vial');
    } else if (
      dosage.includes('ointment') ||
      dosage.includes('cream') ||
      dosage.includes('gel')
    ) {
  ...
  const createMed = useMutation({
    mutationFn: (p: any) =>
      apiFetch('/api/inventory/medicines', {
        method: 'POST',
    ...
    onError: (e: any) =>
      setFormError(e.message || 'Failed to create medicine.'),
  });
  ...
  const categories = useMemo(
    () => [
      'All',
      ...Array.from(
        new Set(medicines.map((m) => m.category || 'Other').filter(Boolean))
      ),
    ],
    [medicines]
  );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredMeds.length / ITEMS_PER_PAGE)
  );
  const pagedMeds = filteredMeds.slice(
  ...
  const lowStockCount = medicines.filter((m) => {
    const q = (batchMap[m.id] || []).reduce(
      (s, b) => s + b.quantity_remaining,
      0
    );
    return q > 0 && q <= m.min_stock_level;
  }).length;
  const outOfStockCount = medicines.filter(
    (m) =>
      (batchMap[m.id] || []).reduce((s, b) => s + b.quantity_remaining, 0) === 0
  ).length;
  ...
  const toggleSelectAll = () =>
    setSelectedIds((prev) =>
      prev.size === pagedMeds.length
        ? new Set()
        : new Set(pagedMeds.map((m) => m.id))
    );

  const drawerBatches = drawerMed ? batchMap[drawerMed.id] || [] : [];
  const drawerTotalQty = drawerBatches.reduce(
    (s, b) => s + b.quantity_remaining,
    0
  );
  const drawerBuyAvg =
    drawerBatches.length > 0
      ? drawerBatches.reduce((s, b) => s + b.purchase_price, 0) /
        drawerBatches.length
      : (drawerMed?.purchase_price ?? 0);
  const drawerSellPrice =
    drawerBatches.length > 0
      ? drawerBatches[drawerBatches.length - 1].selling_price
      : (drawerMed?.selling_price ?? 0);
  const drawerMargin =
    drawerBuyAvg > 0
      ? ((drawerSellPrice - drawerBuyAvg) / drawerBuyAvg) * 100
      : 0;
    ...
    <div className="flex flex-col gap-6 relative">
      ...
      {nearExpiryCount > 0 && (
        <div className="flex items-center justify-between px-5 py-2.5 bg-[#ffb961] text-[#361f00] rounded-xl border border-[#e67e22]/30 -mb-2">
          <div className="flex items-center gap-2.5 text-[13px] font-medium">
            ...
            <span>
              Critical Alert: {nearExpiryCount} batches are expiring within the
              next 30 days.
            </span>
          </div>
          <button className="text-[12px] font-semibold underline hover:opacity-70 cursor-pointer">
            Review Batches
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-[28px] font-bold text-[#191c1e] leading-tight">
        ...
        {[
          {
            ...
            iconBg: 'bg-[#00273b]/10 text-[#00273b]',
            border: '',
          },
          {
            label: 'Low Stock',
          ...
          },
          {
            ...
            border: 'border-l-4 border-l-[#ba1a1a]',
          },
          {
            label: 'Near Expiry',
            val: nearExpiryCount,
          ...
          <div
            key={c.label}
            className={`bg-white rounded-xl p-4 border border-[#e0e3e6] shadow-sm flex items-center gap-4 ${c.border}`}
          >
            <div
              className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${c.iconBg}`}
            >
              {c.icon}
            </div>
            <div>
              <p className="text-[12px] text-[#42474d]">{c.label}</p>
              <p className="text-[22px] font-bold text-[#191c1e] font-display leading-tight">
                {c.val}
              </p>
            </div>
          </div>
      ...
      <div className="bg-white border border-[#e0e3e6] rounded-xl shadow-sm flex flex-col">
        ...
        <div className="p-4 border-b border-[#e0e3e6] flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3 flex-1 flex-wrap">
            ...
            <div className="relative min-w-[220px]">
              <Search
                size={15}
                className="absolute left-3 top-1/2 -translate-y-1/2 text-[#72787e] pointer-events-none"
              />
              <input
          ...
          <button
            onClick={() => refetchMeds()}
            className="p-1.5 rounded-md border border-[#c2c7cd] text-[#42474d] hover:bg-[#f7f9fc] transition-colors cursor-pointer"
            title="Refresh"
          >
            <RefreshCw size={16} />
          </button>
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr>
                <th className="px-4 py-3 bg-[#f2f4f7] border-b border-[#e0e3e6] w-10 text-center sticky top-0 z-10">
                  <input
                    type="checkbox"
                    className="h-4 w-4 rounded border-[#c2c7cd] accent-[#00273b] cursor-pointer"
                    checked={
                      pagedMeds.length > 0 &&
                      selectedIds.size === pagedMeds.length
                    }
                ...
                {[
                  ...
                  <th
                    key={h}
                    className="px-4 py-3 bg-[#f2f4f7] border-b border-[#e0e3e6] text-[11px] font-semibold text-[#42474d] uppercase tracking-wider whitespace-nowrap sticky top-0 z-10"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="text-[13px]">
              {medsLoading ? (
                <tr>
                  <td colSpan={9} className="py-16 text-center text-[#72787e]">
                    <div className="flex flex-col items-center gap-2">
                      <Loader2
                        size={24}
                        className="animate-spin text-[#00273b]"
                      />
                ...
                pagedMeds.map((med) => {
                  ...
                  const activeBat = medBatches.filter(
                    (b) => b.quantity_remaining > 0
                  ).length;
                  const lastBatch = medBatches[medBatches.length - 1];
                  const sellPrice =
                    lastBatch?.selling_price ?? med.selling_price ?? 0;
                  const buyAvg =
                    medBatches.length > 0
                      ? medBatches.reduce((s, b) => s + b.purchase_price, 0) /
                        medBatches.length
                      : (med.purchase_price ?? 0);
                  const margin =
                    buyAvg > 0 ? ((sellPrice - buyAvg) / buyAvg) * 100 : 0;
                      ...
                      <tr
                        className={`border-b border-[#eceef1] cursor-pointer transition-colors ${
                          isSelected
                            ? 'bg-[#e5eeff]'
                            : isExpanded
                              ? 'bg-[#f8f9ff] border-l-2 border-l-[#00273b]'
                              : 'hover:bg-[#f7f9fc]'
                        }`}
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {med.image_url ? (
                              <img
                                src={med.image_url}
                                alt={med.name}
                                className="w-8 h-8 rounded-lg object-cover border border-[#e0e3e6]/60 shrink-0"
                              />
                            ) : (
                            ...
                            <div>
                              ...
                              <div className="text-[11px] text-[#42474d]">
                                {med.generic_name || '—'}
                              </div>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-[#42474d]">
                          {med.category || '—'}
                        </td>
                        <td className="px-4 py-3 font-mono text-[12px] text-[#42474d]">
                          {med.barcode || '—'}
                        </td>
                        <td className="px-4 py-3 w-[180px]">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span
                              className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${status.bg} ${status.color}`}
                            >
                              {status.label}
                            </span>
                          </div>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`px-2 py-1 rounded text-[11px] font-semibold ${activeBat > 0 ? 'bg-[#0f3d57] text-[#80a8c6] border border-[#00273b]/20' : 'bg-[#eceef1] text-[#72787e]'}`}
                          >
                            {activeBat > 0 ? `${activeBat} Active` : 'None'}
                          </span>
                        </td>
                        <td className="px-4 py-3 font-medium text-[#191c1e]">
                          {sellPrice > 0 ? `Rs ${fmt(sellPrice)}` : '—'}
                        </td>
                        <td className="px-4 py-3">
                          {margin > 0 ? (
                            <div>
                              ...
                              <div className="text-[10px] text-[#006d37] font-semibold">
                                margin
                              </div>
                            </div>
                          ) : (
                        ...
                        <tr className="border-b border-[#eceef1] bg-[#f8f9ff]">
                          ...
                          <td colSpan={8} className="pb-4 pt-1 pr-4">
                            <div className="ml-8 bg-white border border-[#e0e3e6] rounded-lg p-3 shadow-sm">
                              <h4 className="text-[10px] font-bold text-[#42474d] uppercase tracking-widest mb-2">
                                Batch Breakdown
                              </h4>
                              {medBatches.length === 0 ? (
                                ...
                                <table className="w-full text-[12px]">
                                  <thead>
                                    <tr className="border-b border-[#eceef1] text-[#42474d]">
                                      ...
                                      <th className="text-right font-semibold pb-1.5">
                                        Qty
                                      </th>
                                      <th className="text-right font-semibold pb-1.5">
                                        Sell Price
                                  ...
                                  <tbody>
                                    {medBatches.map((b) => {
                                      ...
                                      return (
                                        <tr
                                          key={b.id}
                                          className="border-b border-[#eceef1]/50 last:border-b-0"
                                        >
                                          <td className="py-1.5 font-mono">
                                            {b.batch_number}
                                          </td>
                                          <td className={`py-1.5 ${exp.color}`}>
                                            {b.expiry_date}
                                          </td>
                                          <td className="py-1.5 text-right font-semibold">
                                            {b.quantity_remaining}
                                          </td>
                                          <td className="py-1.5 text-right text-[#42474d]">
                                            Rs {fmt(b.selling_price)}
                                          </td>
                                          <td className="py-1.5 text-center">
                                            <span
                                              className={`text-[10px] px-2 py-0.5 rounded font-semibold ${exp.badge}`}
                                            >
                                              {exp.label}
                                            </span>
                                          </td>
        ...
        <div className="px-4 py-3 border-t border-[#e0e3e6] flex items-center justify-between text-[12px] text-[#42474d]">
          <span>
            Showing{' '}
            {Math.min((page - 1) * ITEMS_PER_PAGE + 1, filteredMeds.length)}–
            {Math.min(page * ITEMS_PER_PAGE, filteredMeds.length)} of{' '}
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#00273b] rounded-xl shadow-[0_10px_25px_rgba(15,61,87,0.3)] px-6 py-3 flex items-center gap-5 z-40">
          <div className="flex items-center gap-2 text-white border-r border-white/20 pr-5">
            <div className="w-6 h-6 rounded bg-white text-[#00273b] flex items-center justify-center font-bold text-sm">
              {selectedIds.size}
            </div>
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <button
                key={a.label}
                className="px-3 py-1.5 text-[12px] text-white hover:bg-[#0f3d57] rounded transition-colors cursor-pointer flex items-center gap-1.5"
              >
                {a.icon} {a.label}
              </button>
            ))}
          ...
          <aside className="fixed right-0 top-0 h-screen w-[400px] bg-white border-l border-[#e0e3e6] shadow-[-4px_0_20px_rgba(0,0,0,0.08)] z-50 flex flex-col">
            ...
            <div className="p-4 border-b border-[#e0e3e6] flex items-center justify-between bg-[#f7f9fc]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-display text-[16px] font-bold text-[#191c1e] leading-tight">
                    {drawerMed.name}
                  </h3>
                  <p className="text-[12px] text-[#42474d]">
                    {drawerMed.category}
                    {drawerMed.generic_name
                      ? ` • ${drawerMed.generic_name}`
                      : ''}
                  </p>
                </div>
              ...
              <button
                className="p-1.5 text-[#42474d] hover:bg-[#e0e3e6] rounded-full transition-colors cursor-pointer"
                onClick={() => setDrawerMed(null)}
              >
                <X size={18} />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5">
              ...
              <div className="flex gap-2">
                <button className="flex-1 py-2 bg-[#00273b] text-white rounded-lg text-[12px] font-semibold hover:bg-[#00273b]/90 transition-colors cursor-pointer">
                  Edit Product
                </button>
                <button className="flex-1 py-2 border border-[#00273b] text-[#00273b] rounded-lg text-[12px] font-semibold hover:bg-[#00273b]/5 transition-colors cursor-pointer">
                  Adjust Stock
                </button>
              </div>
              ...
              <div className="bg-[#f7f9fc] rounded-xl p-4 border border-[#e0e3e6]">
                <h4 className="text-[10px] font-bold text-[#42474d] uppercase tracking-widest mb-3">
                  Pricing Information
                </h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[12px] text-[#42474d] mb-1">
                      Unit Selling Price
                    </p>
                    <p className="font-display text-[18px] font-bold text-[#191c1e]">
                      Rs {fmt(drawerSellPrice)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[12px] text-[#42474d] mb-1">
                    ...
                    <p className="text-[16px] font-semibold text-[#42474d]">
                      Rs {fmt(drawerBuyAvg)}
                    </p>
                  </div>
                </div>
                <div className="mt-4 pt-3 border-t border-[#e0e3e6]/60 flex items-center justify-between">
                  <span className="text-[12px] text-[#42474d]">
                    Profit Margin
                  </span>
                  <span className="px-2 py-1 rounded bg-[#6bfe9c] text-[#006d37] font-bold text-[13px]">
                    {drawerMargin.toFixed(1)}%
                  </span>
                </div>
              </div>
                  ...
                  {[
                    {
                      ...
                      val: `${drawerTotalQty} units`,
                      cls: 'font-semibold text-[#191c1e]',
                    },
                    {
                      label: 'Reorder Level',
                      val: `${drawerMed.min_stock_level} units`,
                      cls: 'font-semibold text-[#42474d]',
                    },
                    {
                      label: 'Barcode',
                      val: drawerMed.barcode || '—',
                      cls: 'font-mono text-[12px] text-[#42474d]',
                    },
                  ].map((r) => (
                    <div
                      key={r.label}
                      className="flex justify-between items-center py-2 border-b border-[#eceef1]"
                    >
                  ...
                  <div className="space-y-2">
                    {drawerBatches.map((b) => {
                      ...
                      return (
                        <div
                          key={b.id}
                          className="flex items-center justify-between py-2 border-b border-[#eceef1] text-[12px]"
                        >
                          <div>
                            <div className="font-mono font-semibold text-[#191c1e]">
                              {b.batch_number}
                            </div>
                          ...
                          <div className="text-right">
                            ...
                            <span
                              className={`text-[10px] px-1.5 py-0.5 rounded font-semibold ${exp.badge}`}
                            >
                              {exp.label}
                            </span>
                          </div>
      ...
      {showAddModal && (
        <div
          ...
          aria-modal="true"
          className="fixed inset-0 bg-[#0b1c30]/40 backdrop-blur-sm z-[200] transition-opacity flex items-center justify-center p-4 md:p-ds-margin-desktop overflow-y-auto animate-in fade-in duration-200"
          role="dialog"
        >
          ...
          <div className="relative bg-surface-container-lowest rounded-xl shadow-[0_10px_15px_rgba(15,61,87,0.1)] w-full max-w-3xl my-8 mx-auto flex flex-col max-h-[92vh] border border-outline-variant transform transition-all animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-ds-lg py-ds-md border-b border-outline-variant bg-surface-container-lowest rounded-t-xl shrink-0">
              <h2
                className="font-display text-[20px] font-semibold text-on-surface m-0"
                id="modal-title"
              >
                Add New Product
              </h2>
              <button
                ...
                className="text-on-surface-variant hover:text-on-surface hover:bg-surface-container-low p-2 rounded-full transition-colors flex items-center justify-center cursor-pointer"
                type="button"
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-ds-md">
                  <div className="flex flex-col gap-ds-xs">
                    <label
                      className="font-sans text-[12px] font-semibold tracking-wider text-on-surface-variant uppercase"
                      htmlFor="productName"
                    >
                      Product Name <span className="text-error">*</span>
                    </label>
                    <input
                      className="w-full bg-surface-bright border border-outline-variant rounded-lg px-4 py-2 font-sans text-[14px] text-on-surface placeholder:text-outline focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-shadow"
                      id="productName"
                      name="productName"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-ds-gutter">
                  <div className="flex flex-col gap-ds-xs">
                    ...
                    <div className="relative">
                      <select
                        className="w-full bg-surface-bright border border-outline-variant rounded-lg px-4 py-2 pr-10 font-sans text-[14px] text-on-surface appearance-none focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-shadow cursor-pointer"
                        id="category"
                        name="category"
                        value={medCategory}
                  ...
                  <div className="flex flex-col gap-ds-xs">
                    ...
                    <div className="relative">
                      ...
                      <input
                        className="w-full bg-surface-bright border border-outline-variant rounded-lg pl-10 pr-4 py-2 font-sans text-[14px] text-on-surface placeholder:text-outline focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-shadow"
                        id="barcode"
                        name="barcode"
                        placeholder="e.g. 4901234001"
                        type="text"
                        value={medBarcode}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-ds-gutter">
                  <div className="flex flex-col gap-ds-xs">
                    ...
                    <div className="relative">
                      <select
                        ...
                        name="uom"
                        value={medUom}
                        onChange={(e) => setMedUom(e.target.value)}
                      >
                  ...
                  <div className="flex flex-col gap-ds-xs">
                    ...
                    <input
                      ...
                      value={medMinStock}
                      onChange={(e) =>
                        setMedMinStock(parseInt(e.target.value) || 0)
                      }
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-ds-xs">
                  ...
                  <textarea
                    className="w-full bg-surface-bright border border-outline-variant rounded-lg px-4 py-2 font-sans text-[14px] text-on-surface placeholder:text-outline focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-shadow resize-y"
                    id="description"
                    name="description"
                ...
                <div className="flex flex-col gap-ds-xs">
                  <label className="font-sans text-[12px] font-semibold tracking-wider text-on-surface-variant uppercase">
                    Product Image
                  </label>
                  {medImage ? (
                    ...
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-outline-variant border-dashed rounded-lg bg-surface-bright hover:bg-surface-container-low transition-colors cursor-pointer group relative">
                      ...
                      <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-[14px] text-on-surface-variant justify-center font-sans">
                          ...
                          <p className="pl-1 text-on-surface-variant">
                            or drag and drop
                          </p>
                        </div>
            ...
            <div className="px-ds-lg py-ds-md border-t border-outline-variant bg-surface-container-lowest rounded-b-xl flex justify-end gap-ds-sm shrink-0">
              <button
                className="px-4 py-2 rounded-lg font-sans text-[12px] font-semibold uppercase tracking-wider text-primary border border-primary bg-transparent hover:bg-surface-container-low transition-colors focus:outline-none focus:ring-2 focus:ring-primary/50 cursor-pointer"
                type="button"
                onClick={() => setShowAddModal(false)}
              >
                Cancel
              ...
              <button
                className="px-4 py-2 rounded-lg font-sans text-[12px] font-semibold uppercase tracking-wider text-on-secondary bg-secondary hover:bg-secondary/90 transition-colors focus:outline-none focus:ring-2 focus:ring-secondary/50 flex items-center gap-2 shadow-sm cursor-pointer disabled:opacity-50"
