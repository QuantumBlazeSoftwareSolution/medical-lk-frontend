...
export const usePOSStore = create<POSState>((set) => ({
  ...
  addToCart: (item) => set((state) => {
    ...
    if (existingIndex > -1) {
      ...
      updatedCart[existingIndex].quantity = Math.min(newQty, item.stockAvailable);
  ...
  updateQuantity: (batchId, quantity) => set((state) => ({
    cart: state.cart.map(item => {
      if (item.batchId === batchId) {
        // Clamp between 1 and available stock
        const clampedQty = Math.max(1, Math.min(quantity, item.stockAvailable));
